...
function locate(options, callback) {
  ...
  var branch;
  if (options.branch) {
    branch = BRANCH_MAPPING[options.branch];

    if (!branch) {
      for (var pattern in REGEXP_MAPPING) {
        var re = new RegExp(pattern);
        if (re.test(options.branch)) {
          branch = options.branch.replace(re, REGEXP_MAPPING[pattern]);
          break;
        }
      }
    }
